...
def minibatch(X, Y, mini_batch_size):
    ...
    # And the remaining of those who is not divisible.
...
def L_layer_model(X, Y, layer_dims, m_train, decay_coeff, alpha0, num_epochs, Lambd, mini_batch_size, \
    ...
    for i in range(0, num_epochs):
        ...
        # The regularization cost shall be cleared when a new epoch begin.
        cost_with_regular = 0       
        learning_rate = np.power(decay_coeff, i)*alpha0
        
        # Loop for one single epoch
        for t, mini_batch in enumerate(mini_batches):
            ...
            # Update parameters.
            # Here the T is only for the bias correction. 
            T = t + 1
...
layer_dims = (train_set_x.shape[0], n_h_1, n_h_2, n_h_3, n_h_4, n_y)    # n_h_5, n_h_6, n_h_7, n_h_8, n_h_9, 
Lambd = 1.2                    # Regularization coeff
decay_coeff = 0.995             # Decay coeff for learning_rate
alpha0 = 0.0008                   # Initial learning_rate
mini_batch_size = m_train           # np.power(2, 5)   # Cell for a mini batch
num_epochs = 2000                   # Go through entire batch for 'num_epochs' times
beta_1 = 0.80                       # First moment coeff
...
pickle.dump(parameters, open('well_trained_parameters_NN_cat.txt', 'wb'))


#See how the model works during the test process~
p = predict(test_set_x, test_set_y, parameters, layer_dims, threshold)
